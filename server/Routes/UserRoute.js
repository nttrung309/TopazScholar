...
UserRoute.get("/get-attend/:uid", async (req, res) => {
  ...
  Activity.find({ actID: { $in: userData.attendedActivitiesID } }).then(
    (data) => {
...
UserRoute.post("/login", async (req, res) => {
  ...
  if (isPasswordCorrect) {
    ...
    return res
      .status(200)
      .json({ message: "Đăng nhập thành công", data: data, token: token });
  } else {
...
const CheckValidForSignUp = (data) => {
  ...
  if (!passwordRegex.test(data.password)) {
    checkState.msg =
      "Mật khẩu không hợp lệ. Mật khẩu phải có ít nhất một ký tự in hoa và một số, từ 8 đến 20 ký tự";
    return checkState;
  }
